...
def generateReorderedFileFromBlastResults(infilepath, outfilepath):
	...
	for line in lines:
		if(len(line)>0):
			...
			newline=arr[0]+"\t"+arr[1]+"\t"+arr[10]+"\t"+arr[2]+"\t"+arr[3]+"\t"+arr[5]+"\t"+arr[8]+"\t"+arr[9]+"\t"+arr[6]+"\t"+arr[7]+"\n"
...
def alltoallBlastP(processedPath,filename, outpath, outfmt=6, num_threads=4):
	cmd=["blastp", "-db", os.path.join(processedPath, filename), "-query", os.path.join(processedPath, filename), "-outfmt", str(outfmt), "-out", outpath, "-num_threads", str(num_threads)]
...
def main(name):
	...
	generateReorderedFileFromBlastResults(os.path.join(resultPath,outfilename), os.path.join(procResultPath, processedOutfilename))
